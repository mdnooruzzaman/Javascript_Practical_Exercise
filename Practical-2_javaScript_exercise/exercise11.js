...
/**
  ...
  */
    ...
    const a2 = accounts[0].tags.flat().includes('consectetur');
    console.log(a2);
  ...
  */
    ...
    const a3 = accounts.map(({tags}) => tags.join(',')).join(',');
    console.log(a3);
  ...
  */
    ...
    const a4 = accounts.map(({friends}) => friends.map(({name}) => name))
    console.log(a4);
  ...
  */
    ...
    const a5 = accounts.filter(({balance ,age}) => age<= 30 && parseInt(balance.replace(/\D/g , '')) > 1500)
                .map(({friends}) => friends)
                .flat();
