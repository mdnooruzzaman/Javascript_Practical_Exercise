 ...
 */
  ...
  const email = users.every(user => /[^@]+@[^\.]+\..+/.test(user.email) )
  console.log(email);
 ...
 */
 const ip_add = users.filter(({ip_address}) => ip_address.split('.')[0] < 128);
 console.log(ip_add);
 ...
 */
 const findex = users.findIndex(({ip_address}) => {
     const firstOctate = ip_address.split('.')[0];
     return firstOctate >= 128 && firstOctate < 192;
 });
 ...
 */
 ...
 const chk = users.filter(({email}) => email.split('@')[1] == 'soundcloud.com');
 console.log(chk);
 //another way

 const chk1 = users.find(({email}) => email.includes('soundcloud'));
 console.log(chk1);
